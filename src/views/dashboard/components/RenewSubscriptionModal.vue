...
const onSubmit = form.handleSubmit(async (values) => {
  ...
  try {
    ...
    }).eq('email', subscriber.value.email)
...
<template>
  <section class="">
    ...
    <form @submit="onSubmit" class="mt-3" v-if="tab === 'search'">
      <FormField v-slot="{ componentField }" name="customer_name">
        <FormItem>
          <FormLabel>Customer Name</FormLabel>
          <FormControl>
            <Input type="text" placeholder="John Doe" v-bind="componentField" v-model="subscriber.customer_name" class="text-black" disabled />
          </FormControl>
          <FormMessage />
        </FormItem>
      </FormField>
